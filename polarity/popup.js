const sentiment=document.getElementById("findsentiment");
let newsdisplay=document.getElementById("newsdisplay");
let sentimentdisplay=document.getElementById("sentimentdisplay");
...
sentiment.addEventListener("click",async () => {

  const [tab]=await chrome.tabs.query({active: true,currentWindow:true});

  if(tab.url.includes("ndtv.com/india-news"))
  {
      sentimentfind(tab.url);
  }
  else if(tab.url.includes("timesofindia.indiatimes.com/india/"))
  {
      timeofindia(tab.url);
  }
})
...
function sentimentfind(tab){
    var title=tab.split("india-news/")[1];
    var t=title.replace(/-/g, " ");
    var index=t.lastIndexOf(" ");
    t=t.substring(0,index);
    console.log(t);
    const result = SentimentIntensityAnalyzer.polarity_scores(t);
    console.log(result);
   
    if(result['compound']>=0.5){
        console.log("Positive");
        newsdisplay.textContent=t;
        sentimentdisplay.textContent='Positive';
    }
    else if(result['compound']<=-0.5){
        console.log("Negative");
        newsdisplay.textContent=t;
        sentimentdisplay.textContent='Negative';
        //chrome.runtime.sendMessage({news:t,sentiment:"Negative"});
    }
    else{ 
        console.log("Neutral");
        newsdisplay.textContent=t;
        sentimentdisplay.textContent='Neutral';
        //chrome.runtime.sendMessage({news:t,sentiment:"Neutral"});
    }
    
}

function timeofindia(tab){
   let title=tab.split("india/")[1];
   var index=title.indexOf("/");
   title=title.substring(0,index);
   title=title.replace(/-/g," ");
   console.log(title);
   let result = SentimentIntensityAnalyzer.polarity_scores(title);
   console.log(result);

    if(result['compound']>=0.5){
        console.log("Positive");
        newsdisplay.textContent=title;
        sentimentdisplay.textContent='Positive';
    }
    else if(result['compound']<=-0.5){
        console.log("Negative");
